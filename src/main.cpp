...
uint256 hashGenesisBlock("0x32f3a2c560499f2600108f5b10dadcfd0c000aa1d6fef25b26d0871d72e607a9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // fuckcoin: starting difficulty is 1 / 2^12
...
{
    // fuckcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
if(nHeight == 69) //bounties, bonuses, get the FUCKcoin going
        { 
                nSubsidy = 3818278 * COIN;
        } 

if(nHeight == 169) //bounties, bonuses, get the FUCKcoin going
        { 
                nSubsidy = 696969 * COIN;
        } 
// added 3818278 (coins to reach 100000000)
// added 696969 (coins to reach 100696969)

    // Subsidy is cut in half every 696969 blocks, which will occur approximately every 556.6 days
    nSubsidy >>= (nHeight / 696969); // fuckcoin: 696969 blocks in 556.6 days

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 69 * 60; // fuckcoin: 69 minutes
static const int64 nTargetSpacing = 69; // fuckcoin: 69 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // fuckcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // fuckcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // fuckcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9ca6cb7d086f01a72e91c1b35a831b715f3a8ac7c45a91ec93df8f1c13797581");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "How Sex Affects Intelligence, and Vice Versa";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1389920669;
        block.nBits    = 0x1e0fffff;
        block.nNonce   = 850900;

        if (fTestNet)
        {
            block.nTime    = 1389920600;
            block.nNonce   = 44259;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x537b5010fe44a2dd680dd345f9554d4f9e99154b34ef03e1303698d5d9ed2cdb"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // fuckcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("fuckcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("fuckcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static fuckcoinMiner(CWallet *pwallet)
{
    printf("fuckcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running fuckcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("fuckcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&fuckcoinMiner, pwallet));
}
